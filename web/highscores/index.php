...
<nav class="navbar navbar-expand-lg navbar-light bg-light">
  ...
  <div class="collapse navbar-collapse" id="navbarNav">
    <ul class="navbar-nav">
      <li class="nav-item">
        <a class="nav-link active" href="index.php"><i class="fas fa-trophy"></i> <?php echo $navleader; ?></a>
      </li>
      <li class="nav-item">
        <a class="nav-link" href="serverstats.php"><i class="fas fa-globe-americas"></i> <?php echo $navserver; ?></a>
      </li>
      <li class="nav-item">
        <a class="nav-link" href="player.php"><i class="fas fa-search"></i> <?php echo $navsearch; ?></a>
      </li>
    </ul>
...
<div class="jumbotron text-center">
  ...
  <h1><i class="fas fa-trophy"></i> <?php echo $navleaderheader; ?></h1>

  <p><?php echo $leaderdesc; ?></p> 
<?php if ($cache) { ?>
<a href="javascript:window.top.location.reload(false)" class="continue">Clear Cache</a>
<?php } ?>
 <?php if ($resolution) {  ?>
 <p id="res">Get Resolution</p>
 <script>
	document.getElementById("res").onclick = function() {getResolution()};
    function getResolution() {
        document.getElementById("res").innerHTML = (screen.width + "x" + screen.height.toString());
...
<?php /*These tables here are in most cases the ones you would enable/disabled. Ember is only for the ones who own it, Wallet is for darkrp & Utime is if you want to use your existing utime database.*/ ?> 

<?php
if ($timeplayed) {
	include 'tables/timeplayed.php';
}
if ($ember){
	 include 'tables/ember.php'; 

}
if ($darkrptable) {
 include 'tables/wallet.php'; 
}
 
	if ($utime) {
 include 'tables/utime.php';  
	}
if ($kdr) {
 include 'tables/kdr.php';
}
 ?> 
<br>
<?php
if ($kills) {
 include 'tables/kill.php'; 
}
if ($deaths) {
 include 'tables/death.php'; 
}
if ($headshots) {
 include 'tables/headshot.php';
}
 mysqli_close($link); ?>
<div>
</body>
